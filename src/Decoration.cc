...
QRect Decoration::centerRect() const
{
	...
	const int leftOffset =
		m_leftButtons->geometry().right() + (leftButtonsVisible ? settings()->smallSpacing() : 0);

	const bool rightButtonsVisible = !m_rightButtons->buttons().isEmpty();
	const int rightOffset =
		m_rightButtons->geometry().width() + (rightButtonsVisible ? settings()->smallSpacing() : 0);

	return titleBarRect().adjusted(leftOffset, 0, -rightOffset, 0);
}
...
void Decoration::init()
{
	...
	auto repaintTitleBar = [this] { update(titleBar()); };

	m_leftButtons = new KDecoration2::DecorationButtonGroup(
		KDecoration2::DecorationButtonGroup::Position::Left, this, &Button::create);

	m_rightButtons = new KDecoration2::DecorationButtonGroup(
		KDecoration2::DecorationButtonGroup::Position::Right, this, &Button::create);

	m_menuButtons = new AppMenuButtonGroup(this);
	connect(m_menuButtons, &AppMenuButtonGroup::menuUpdated, this,
			&Decoration::updateButtonsGeometry);
	connect(m_menuButtons, &AppMenuButtonGroup::opacityChanged, this, repaintTitleBar);
	connect(m_menuButtons, &AppMenuButtonGroup::alwaysShowChanged, this, repaintTitleBar);
	m_menuButtons->updateAppMenuModel();

	connect(decoratedClient, &KDecoration2::DecoratedClient::widthChanged, this,
			&Decoration::updateTitleBar);
	connect(decoratedClient, &KDecoration2::DecoratedClient::widthChanged, this,
			&Decoration::updateButtonsGeometry);
	connect(decoratedClient, &KDecoration2::DecoratedClient::maximizedChanged, this,
			&Decoration::updateButtonsGeometry);

	connect(decoratedClient, &KDecoration2::DecoratedClient::adjacentScreenEdgesChanged, this,
			&Decoration::updateBorders);
	connect(decoratedClient, &KDecoration2::DecoratedClient::maximizedHorizontallyChanged, this,
			&Decoration::updateBorders);
	connect(decoratedClient, &KDecoration2::DecoratedClient::maximizedVerticallyChanged, this,
			&Decoration::updateBorders);
	connect(decoratedClient, &KDecoration2::DecoratedClient::shadedChanged, this,
			&Decoration::updateBorders);

	connect(decoratedClient, &KDecoration2::DecoratedClient::captionChanged, this, repaintTitleBar);
	connect(decoratedClient, &KDecoration2::DecoratedClient::activeChanged, this, repaintTitleBar);
	...
	connect(this, &KDecoration2::Decoration::sectionUnderMouseChanged, this,
			&Decoration::onSectionUnderMouseChanged);
	...
	connect(settings().data(), &KDecoration2::DecorationSettings::reconfigured, this,
			&Decoration::reconfigure);
	connect(m_internalSettings.data(), &InternalSettings::configChanged, this,
			&Decoration::reconfigure);
	...
	connect(settings().data(), &KDecoration2::DecorationSettings::borderSizeChanged, this,
			&Decoration::updateBorders);
	connect(settings().data(), &KDecoration2::DecorationSettings::fontChanged, this,
			&Decoration::updateBorders);
	connect(settings().data(), &KDecoration2::DecorationSettings::spacingChanged, this,
			&Decoration::updateBorders);
}
...
void Decoration::setButtonGroupHeight(KDecoration2::DecorationButtonGroup *buttonGroup,
									  int buttonHeight)
{
...
void Decoration::updateButtonsGeometry()
{
	...
	m_rightButtons->setPos(
		QPointF(size().width() - rightOffset - m_rightButtons->geometry().width(), 0));
	...
	if (!m_menuButtons->buttons().isEmpty()) {
		...
		const QRect availableRect = centerRect().adjusted(0, 0, -captionOffset, 0);
...
void Decoration::setButtonGroupAnimation(KDecoration2::DecorationButtonGroup *buttonGroup,
										 bool enabled, int duration)
{
	for (int i = 0; i < buttonGroup->buttons().length(); i++) {
		auto *button = qobject_cast<Button *>(buttonGroup->buttons().value(i));
...
void Decoration::updateShadow()
{
	...
	if (!s_cachedShadow.isNull() && (s_shadowColor == shadowColor) &&
		(s_shadowSizePreset == shadowSizePreset) && (s_shadowStrength == shadowStrengthInt)) {
	...
	BoxShadowHelper::boxShadow(&painter, box, params.shadow1.offset, params.shadow1.radius,
							   withOpacity(shadowColor, params.shadow1.opacity * shadowStrength));

	// Draw the "contrast" shadow.
	BoxShadowHelper::boxShadow(&painter, box, params.shadow2.offset, params.shadow2.radius,
							   withOpacity(shadowColor, params.shadow2.opacity * shadowStrength));

	// Mask out inner rect.
	const QMargins padding =
		QMargins(shadowSize - params.offset.x(), shadowSize - params.offset.y(),
				 shadowSize + params.offset.x(), shadowSize + params.offset.y());
...
QPoint Decoration::windowPos() const
{
	...
	if (KWindowSystem::isPlatformX11()) {
		...
		/*
		 * get root position matching position need to use xcb because the embedding of the widget
		 * breaks QT's mapToGlobal and other methods
		 */
		auto connection(QX11Info::connection());
		xcb_get_geometry_cookie_t cookie(xcb_get_geometry(connection, windowId));
		ScopedPointer<xcb_get_geometry_reply_t> reply(
		...
		if (reply) {
			...
			xcb_translate_coordinates_cookie_t coordCookie(xcb_translate_coordinates(
				connection, windowId, reply.data()->root, -reply.data()->border_width,
				-reply.data()->border_width));

			ScopedPointer<xcb_translate_coordinates_reply_t> coordReply(
				xcb_translate_coordinates_reply(connection, coordCookie, nullptr));
...
bool Decoration::dragMoveTick(const QPoint pos)
{
	...
	// qCDebug(category) << "    diff" << diff << "mL" << diff.manhattanLength() << "sDD" <<
...
void Decoration::sendMoveEvent(const QPoint pos)
{
	...
	QPoint globalPos = windowPos() - QPoint(0, titleBarHeight()) + pos;
	...
	if (KWindowSystem::isPlatformX11()) {
		...
		if (!m_moveResizeAtom) {
			...
			xcb_intern_atom_cookie_t cookie(
				xcb_intern_atom(connection, false, atomName.size(), qPrintable(atomName)));
		...
		xcb_send_event(connection, false, windowId, XCB_EVENT_MASK_BUTTON_RELEASE,
...
void Decoration::paintFrameBackground(QPainter *painter, const QRect &repaintRegion) const
{
	...
	painter->setClipRect(0, borderTop(), size().width(), size().height() - borderTop(),
						 Qt::IntersectClip);
...
inline int colorLimit(int a, int b)
{
	int result = a + b;
	if (result < 0) {
		result = 0;
...
QColor Decoration::adjustedColor(KDecoration2::ColorRole role) const
{
	...
	if (lightness != 1.0) {
		int offset = qRound((lightness - 1.0) * 255.0);
		QRgb rgb = color.rgba();
		color = QColor::fromRgb(colorLimit(qRed(rgb), offset), colorLimit(qGreen(rgb), offset),
								colorLimit(qBlue(rgb), offset), qAlpha(rgb));
	}
...
{
	const auto *decoratedClient = client().toStrongRef().data();
	const auto group = decoratedClient->isActive() ? KDecoration2::ColorGroup::Active
...
void Decoration::paintCaption(QPainter *painter, const QRect &repaintRegion) const
{
	...
	const int textWidth =
		settings()->fontMetrics().boundingRect(decoratedClient->caption()).width();
	...
	const int menuButtonsWidth =
		m_menuButtons->geometry().width() + (appMenuVisible ? appMenuCaptionSpacing() : 0);

	const QRect availableRect =
		centerRect().adjusted((m_menuButtons->alwaysShow() ? menuButtonsWidth : 0), 0, 0, 0);
	...
	const QString caption = painter->fontMetrics().elidedText(decoratedClient->caption(),
															  Qt::ElideMiddle, captionRect.width());
	...
	if (m_menuButtons->buttons().isEmpty()) {
		...
		if (!m_menuButtons->alwaysShow()) { // caption fades away revealing menu
			...
			painter->setPen(titleBarForegroundColor());
		} else if (m_menuButtons
					   ->overflowing()) { // hide caption leaving "whitespace" to easily grab.
