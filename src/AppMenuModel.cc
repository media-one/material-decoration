 ...
 *License or (at your option) version 3 or any later version accepted by the membership of KDE e.V.
 *(or its successor approved by the membership of KDE e.V.), which shall act as a proxy defined in
 *Section 14 of version 3 of the license.
 *
...
void AppMenuModel::x11Init()
{
	...
	void (KWindowSystem::*myWindowChangeSignal)(WId window, NET::Properties properties,
												NET::Properties2 properties2) =
		&KWindowSystem::windowChanged;
	connect(KWindowSystem::self(), myWindowChangeSignal, this, &AppMenuModel::onWindowChanged);
...
void AppMenuModel::onActiveWindowChanged(WId id)
{
	...
	if (KWindowSystem::isPlatformX11()) {
		...
		auto getWindowPropertyString = [c](WId id, const QByteArray &name) -> QByteArray {
			...
			auto propertyCookie =
				xcb_get_property(c, false, id, s_atoms[name], XCB_ATOM_STRING, 0, MAX_PROP_SIZE);
		...
		auto updateMenuFromWindowIfHasMenu = [this, &getWindowPropertyString](WId id) {
			const QString serviceName =
				QString::fromUtf8(getWindowPropertyString(id, s_x11AppMenuServiceNamePropertyName));
			const QString menuObjectPath =
				QString::fromUtf8(getWindowPropertyString(id, s_x11AppMenuObjectPathPropertyName));
		...
		if (!filterChildren()) {
			...
			KWindowInfo transientInfo =
				KWindowInfo(info.transientFor(), NET::WMState | NET::WMWindowType | NET::WMGeometry,
							NET::WM2TransientFor);
...
void AppMenuModel::updateApplicationMenu(const QString &serviceName, const QString &menuObjectPath)
{
	...
	connect(m_importer.data(), &DBusMenuImporter::actionActivationRequested, this,
			[this](QAction *action) {
				// TODO submenus
				if (!m_menuAvailable || !m_menu) {
					return;
