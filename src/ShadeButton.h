...
public:
	...
	{
		QObject::connect(decoratedClient, &KDecoration2::DecoratedClient::shadeableChanged, button,
						 &Button::setVisible);
						  ...
						  const qreal gridUnit)
	{
		...
		if (button->isChecked()) {
			...
			painter->drawLine(QPointF(0, 2) * gridUnit, QPointF(10, 2) * gridUnit);
			button->setPenWidth(painter, gridUnit, 1.25);
			painter->drawPolyline(QVector<QPointF>{QPointF(0.5, 5.25) * gridUnit,
			...
			painter->drawLine(QPointF(0, 2) * gridUnit, QPointF(10, 2) * gridUnit);
			button->setPenWidth(painter, gridUnit, 1.25);
			painter->drawPolyline(QVector<QPointF>{QPointF(0.5, 9.75) * gridUnit,
