	...
	, m_opacity(1)
{
	...
	connect(this, &AppMenuButtonGroup::currentIndexChanged, this,
			&AppMenuButtonGroup::updateShowing);
	...
	connect(decoratedClient, &KDecoration2::DecoratedClient::hasApplicationMenuChanged, this,
			&AppMenuButtonGroup::updateAppMenuModel);
	connect(this, &AppMenuButtonGroup::requestActivateIndex, this, &AppMenuButtonGroup::trigger);
	connect(this, &AppMenuButtonGroup::requestActivateOverflow, this,
			&AppMenuButtonGroup::triggerOverflow);
}
...
void AppMenuButtonGroup::setShowing(bool value)
{
	if (m_showing != value) {
		m_showing = value;
		// qCDebug(category) << this << "setShowing" << m_showing << "alwaysShow" << m_alwaysShow <<
		// "currentIndex" << m_currentIndex << "opacity" << m_opacity;
	...
	if (m_appMenuModel) {
		...
		for (int row = 0; row < m_appMenuModel->rowCount(); row++) {
			...
			const QString itemLabel =
				m_appMenuModel->data(index, AppMenuModel::MenuRole).toString();
...
void AppMenuButtonGroup::trigger(int buttonIndex)
{
	...
	if (actionMenu && deco) {
		...
		connect(actionMenu, &QMenu::aboutToHide, this, &AppMenuButtonGroup::onMenuAboutToHide,
				Qt::UniqueConnection);
	}
}
...
bool AppMenuButtonGroup::eventFilter(QObject *watched, QEvent *event)
{
	...
	if (event->type() == QEvent::KeyPress) {
		...
		if (appMenuButton) {
			if ((m_currentIndex != appMenuButton->buttonIndex()) && appMenuButton->isVisible() &&
...
void AppMenuButtonGroup::onShowingChanged(bool showing)
{
	if (m_animationEnabled) {
		QAbstractAnimation::Direction dir =
			showing ? QAbstractAnimation::Forward : QAbstractAnimation::Backward;
