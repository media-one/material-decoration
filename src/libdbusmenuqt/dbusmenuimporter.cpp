...
public:
	...
	QDBusPendingCallWatcher *refresh(int id)
	{
		...
		QObject::connect(watcher, &QDBusPendingCallWatcher::finished, q,
						 &DBusMenuImporter::slotGetLayoutFinished);
	...
	/**
	 * Init all the immutable action properties here TODO: Document immutable properties?
	 *
	 * Note: we remove properties we handle from the map (using QMap::take() instead of
	 *QMap::value()) to avoid warnings about these properties in updateAction()
	 */
	...
	/**
	 * Update mutable properties of an action. A property may be listed in requestedProperties but
	 *not in map, this means we should use the default value for this property.
	 *
	...
	void updateActionIconByData(QAction *action, const QVariant &value)
	{
		...
		if (!pix.loadFromData(data)) {
			qDebug(DBUSMENUQT) << "Failed to decode icon-data property for action"
							   << action->text();
			action->setIcon(QIcon());
			return;
		}
	...
	void slotItemsPropertiesUpdated(const DBusMenuItemList &updatedList,
	...
	, d(new DBusMenuImporterPrivate)
{
	...
	connect(d->m_pendingLayoutUpdateTimer, &QTimer::timeout, this,
			&DBusMenuImporter::processPendingLayoutUpdates);

	connect(d->m_interface, &DBusMenuInterface::LayoutUpdated, this,
			&DBusMenuImporter::slotLayoutUpdated);
	connect(d->m_interface, &DBusMenuInterface::ItemActivationRequested, this,
			&DBusMenuImporter::slotItemActivationRequested);
	connect(d->m_interface, &DBusMenuInterface::ItemsPropertiesUpdated, this,
...
void DBusMenuImporterPrivate::slotItemsPropertiesUpdated(const DBusMenuItemList &updatedList,
														 const DBusMenuItemKeysList &removedList)
{
	...
	for (const DBusMenuLayoutItem &dbusMenuItem : rootItem.children) {
		...
		if (it == d->m_actionForId.end()) {
			...
			connect(action, &QObject::destroyed, this,
					[this, id]() { d->m_actionForId.remove(id); });

			connect(action, &QAction::triggered, this, [id, this]() { sendClickedEvent(id); });

			if (QMenu *menuAction = action->menu()) {
				connect(menuAction, &QMenu::aboutToShow, this,
						&DBusMenuImporter::slotMenuAboutToShow, Qt::UniqueConnection);
			}
...
void DBusMenuImporter::updateMenu(QMenu *menu)
{
	...
	// Firefox deliberately ignores "aboutToShow" whereas Qt ignores" opened", so we'll just send
	// both all the time...
