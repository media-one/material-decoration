...
public:
	...
	static void paintIcon(Button *button, QPainter *painter, const QRectF &iconRect, qreal gridUnit)
	{
		if (button->isChecked()) {
			...
			painter->drawPolygon(QVector<QPointF>{iconRect.bottomLeft(),
			...
			painter->drawPolyline(
				QVector<QPointF>{iconRect.topLeft() + QPointF(offset, offset),
								 iconRect.topLeft() + QPointF(offset, 0), iconRect.topRight(),
